...
import { Plus, Search, FileText, Globe, Sparkles, ChevronDown, PanelRightClose, History, PanelLeft } from "lucide-react";
...
const SourcesPanel = ({
  ...
  if (!showSourcesPanel) {
    return (
      <div className={`hidden md:flex bg-card border-r border-border flex flex-col items-center py-4 w-14 h-full shrink-0 ${className}`}>
        {/* Toggle Icon */}
        <Button
          variant="ghost"
          size="icon"
          ...
          <div className="flex flex-col gap-3">
            {sources.slice(0, 5).map((source) => (
              <div
                ...
                title={source.name}
              >
                <div className="text-[8px] font-bold text-destructive leading-none mb-0.5">PDF</div>
            ...
            {Object.keys(groupedCitations).length > 0 && (
              <div className="w-8 h-8 rounded-md bg-secondary/50 flex flex-col items-center justify-center border border-border/50">
                <div className="text-[8px] font-bold text-primary leading-none mb-0.5">CIT</div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {sources.length > 0 || Object.keys(groupedCitations).length > 0 ? (
          <div className="space-y-3">
            ...
            {(Object.entries(groupedCitations) as [string, Citation[]][]).map(([docName, docCitations]) => (
